    ...
    <Sidebar class="sidebar w-50 pb-4" v-model:visible="show" @show="willAppear()" position="right">
        ...
        <div class="pa-4 d-flex flex-column sidebar-content pb-12">
            ...
            <div class="d-flex position-absolute bottom-0 py-2 bg-white w-100">
                <VBtn prepend-icon="mdi-check" class="black-button mr-4" @click="updateInfo">
                    Изменить
                </VBtn>
...
const willAppear = () => {
    ...
    infoStore.newInfo.isHidden = false
    infoStore.newInfo.image.url = ''

    infoStore.newInfo.id = infoStore.selectEditInfo?.id
    infoStore.newInfo.title = infoStore.selectEditInfo?.title
    infoStore.newInfo.body = infoStore.selectEditInfo?.body
    infoStore.newInfo.important = infoStore.selectEditInfo?.important
    infoStore.newInfo.category = infoStore.selectEditInfo?.category
    infoStore.newInfo.image.url = infoStore.selectEditInfo?.image
    infoStore.newInfo.source = infoStore.selectEditInfo?.source
}
...
async function updateInfo() {
    ...
    if (infoStore.newInfo?.image?.isChanged) {
        ...
        const coverResponse: any = await infoStore.updateInfoCover(infoStore.newInfo?.id, infoStore.newInfo?.image?.file)
    ...
    // UPADTE ARTICLE
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const response: any = await infoStore.updateInfo()
