<template>
    <Sidebar class="sidebar w-50 pb-4" v-model:visible="show" @show="willAppear()" position="right">
        <div class="sidebar-header py-4 px-8 fixed ">
            ...
            <h2 class="sidebar-header-title"> Отзывы на "{{ eventsStore.selectFeedbackEvent.name }}"</h2>
        ...
        <div class="pa-4 d-flex flex-column sidebar-content">
            ...
            <VItemGroup class="mt-1" v-if="eventsStore.selectEventFeedbacks?.length > 0">
                <VRow class="d-flex my-2 mx-6 rounded-lg" v-for="(feedback) in eventsStore.selectEventFeedbacks"
                    ...
                    <div class="bg-color2 d-flex flex-column rounded-lg">
                        <div class="d-flex pt-2">
                            <div class="d-flex">
                                ...
                                <!-- DELETE DIALOG -->
                                <VDialog v-model="deleteFeedbackDialog" max-width="400" persistent>
                                    <template v-slot:activator="{ props: deleteFeedbackDialog }">
                                        <VIcon icon="mdi-delete-outline" class="cursor-pointer" color="red"
                                            style="right: 34px; position: absolute;" v-bind="deleteFeedbackDialog" />
                                    </template>
                                    ...
                                    <VCard class="pa-6">
                                        ...
                                        <template v-slot:actions>
                                            <VBtn @click="deleteFeedbackDialog = false">
                                                Отмена
                                            </VBtn>
...
">
                ...
                <LoadMarker top="-1000px"
                    @load="eventsStore.selectEventFeedbacksPage++; eventsStore.getEventFeedbacks(eventsStore.selectFeedbackEvent.id)" />
            </VItemGroup>
...
const willAppear = async () => {
    eventsStore.selectEventFeedbacksPage = 1
    await eventsStore.getEventFeedbacks(eventsStore.selectFeedbackEvent.id)
}
...
const deleteFeedback = async (feedbackId: number) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const response: any = await eventsStore.deleteEventFeedback(feedbackId)
