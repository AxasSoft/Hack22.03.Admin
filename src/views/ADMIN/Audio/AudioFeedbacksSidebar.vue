<template>
    <Sidebar class="sidebar w-50 pb-4" v-model:visible="show" @show="willAppear()" position="right">
        <div class="sidebar-header py-4 px-8 fixed ">
            ...
            <h2 class="sidebar-header-title"> Отзывы на "{{ audiosStore.selectFeedbackAudio.name }}"</h2>
        ...
        <div class="pa-4 d-flex flex-column sidebar-content">
            ...
            <VItemGroup class="mt-1" v-if="audiosStore.selectAudioFeedbacks?.length > 0">
                <VRow class="d-flex my-2 mx-6 rounded-lg" v-for="(feedback) in audiosStore.selectAudioFeedbacks"
...
">
                ...
                <LoadMarker top="-1000px"
                    @load="audiosStore.selectAudioFeedbacksPage++; audiosStore.getAudioFeedbacks(audiosStore.selectFeedbackAudio.id)" />
            </VItemGroup>
...
const willAppear = async () => {
    audiosStore.selectAudioFeedbacksPage = 1
    await audiosStore.getAudioFeedbacks(audiosStore.selectFeedbackAudio.id)
}
...
const deleteFeedback = async (feedbackId: number) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const response: any = await audiosStore.deleteAudioFeedback(feedbackId)
